...
- [x] **DASH-04**: Dashboard shows setup history with lifecycle outcomes for every emitted signal
- [x] **DASH-05**: Dashboard shows performance stats (win rate, PF, expectancy, equity curve in R) aggregate and per symbol
- [x] **DASH-06**: Dashboard shows a data/pipeline health strip (last bar time per feed, MT5 connection status, errors)
